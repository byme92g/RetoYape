...
public class KafkaConsumer<T> : BackgroundService, IKafkaConsumer<T> where T : class
{
    ...
    public KafkaConsumer(IConfiguration configuration, IKafkaConsumerHandler<T> handler, ILogger<KafkaConsumer<T>> logger, IServiceScopeFactory serviceScopeFactory)
    {
        _handler = handler;
        _logger = logger;
        ...
        _consumer.Subscribe("transaction-topic");
        _serviceScopeFactory = serviceScopeFactory;
    }
